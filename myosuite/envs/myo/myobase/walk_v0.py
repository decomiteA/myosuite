...
class WalkEnvV4(BaseV0):
    ...
    def get_obs_dict(self, sim):
        ...
        obs_dict['muscle_velocity'] = self.muscle_velocities()
        obs_dict['muscle_force'] = self.muscle_forces()
        obs_dict['hip_period'] = self.hip_period.copy()
    ...
    def get_reward_dict(self, obs_dict):
        #vel_reward = self._get_vel_reward()
        vel_reward = self._get_vel_reward_rect()
        cyclic_hip = self._get_cyclic_rew()
        ref_rot = self._get_ref_rotation_rew()
        joint_angle_rew = self._get_joint_angle_rew(['hip_adduction_l', 'hip_adduction_r', 'hip_rotation_l',
                                                       'hip_rotation_r'])
    ...
    def _get_vel_reward_rec(self):
        """
        Gaussian that incentivizes a walking velocity within a given rectangular window
        The reward outside of this window is similar to what we have above
        @ Antoine de Comite
        """
        vel = self._get_com_velocity()
        lb, ub = 1.0, 1.8
        if (vel[1] < ub) and (vel[1] > lb):
            return np.exp(-np.square(self.target_x_vel - vel[0]))
        else:
            min_dist = min(np.abs(vel[1] - lb),np.abs(vel[1] - ub))
            return np.exp(-np.square(min_dist)) + np.exp(-np.square(self.target_x_vel - vel[0]))
